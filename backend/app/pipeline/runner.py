...
def run_pipeline(
    ...
    s1_cer, _ = _cer(ground_truth_text, stage1_text)
    s1_wer, _ = _wer(ground_truth_text, stage1_text)

    # Stage 2: Spell Correction
    sc = spell_correct(stage1_text)
    s2_text = sc["text"]
    s2_cer, _ = _cer(ground_truth_text, s2_text)
    s2_wer, _ = _wer(ground_truth_text, s2_text)
    recovery = error_recovery_rate(s1_cer, s2_cer)

    # Stage 3: Summarization
    summ = extractive_summarize(s2_text)
    s3_text = summ["text"]
    rouge = compute_rouge(ground_truth_summary, s3_text)
